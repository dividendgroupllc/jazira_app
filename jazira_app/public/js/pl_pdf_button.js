// pl_pdf_button.js
// "Profit and Loss Statement" report'iga "Jazira PDF" tugmasini qo'shadi.
...
(function () {
	...
	var BTN_CLASS = "btn-jazira-pl-pdf";
	...
	function _on_click() {
		...
		frappe.call({
			method: "jazira_app.jazira_app.api.pl_pdf_api.generate_pl_pdf",
	...
	function _inject_button(report) {
		...
		report.page
			.add_inner_button(__("Jazira PDF"), _on_click)
		...
		function _install_trap() {
				...
				set: function (newVal) {
					...
					if (newVal && typeof newVal === "object") {
						try {
							...
							console.error("[Jazira] patch_report error:", e);
						}
					}
				},
			});
			...
			if (_storage[REPORT_NAME]) {
				try {
					...
					console.error("[Jazira] initial patch error:", e);
				}
			}
		}
