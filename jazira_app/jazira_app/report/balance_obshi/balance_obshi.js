...
function jazira_balance_obshi_pdf_button(report) {
    if (!report || !report.page || !report.page.inner_toolbar) return;
    report.page.inner_toolbar.removeClass("hidden-xs hidden-md");
    if (report.page.inner_toolbar.find(".btn-balance-obshi-pdf").length) return;
    ...
    const $btn = report.page.add_inner_button(__("PDF"), function () {
        ...
        window.open("/api/method/jazira_app.jazira_app.report.balance_obshi.balance_obshi_pdf.generate_balance_obshi_pdf"
            + "?filters=" + encodeURIComponent(JSON.stringify(filters)));
    });
    if ($btn && $btn.addClass) $btn.addClass("btn-balance-obshi-pdf").addClass("btn-primary");
}

frappe.query_reports["Balance Obshi"] = {
    "filters": [
        {
            "fieldname": "from_date",
            "label": __("Дан"),
    ...
    onload: function (report) {
        jazira_balance_obshi_pdf_button(report);
    },
    after_datatable_render: function () {
        jazira_balance_obshi_pdf_button(frappe.query_report);
    },
